...
PARENT_PATH="$(pwd -P)"
popd
AUTH_SECRET_NAME="${AUTH_SECRET_NAME:-"hpecp-ext-auth-secret"}"
AUTH_SECRET_NAMESPACE="${AUTH_SECRET_NAMESPACE:-"hpecp"}"
DEX_CONFIG_FILE="${PARENT_PATH}/template.yaml"
	...
	if [[ "$group" == *"::::"* ]]; then
        ...
        for one_of_groups in "${groups[@]}"; do
        	...
        	if [[ "$group_root_dn" != "$group_base_dn" ]]; then
        		echo "[WARNING]   'groups': \"$(echo "$group" | sed 's/;/::::/')\" contains groups with different base DN"
        	fi
        done
...
function main() {
	...
	local yaml="/tmp/dex-config.yaml"
	echo "${config}" > "$yaml"
    kubectl create secret generic dex-config-secret -n auth --from-file config.yaml="$yaml" --dry-run -o yaml | kubectl apply -f - || exit 1
    rm "$yaml"
