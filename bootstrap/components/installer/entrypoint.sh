...
deploy_cert_manager()
{
    ...
    # waiting for cert-manager being deployed
    while [ -z "$(kubectl get apiservices.apiregistration.k8s.io v1beta1.webhook.cert-manager.io --ignore-not-found)" ]; do echo "waiting for webhook apiservice being deployed" && sleep 5; done
    kubectl wait --for=condition=Available --timeout=600s apiservice v1beta1.webhook.cert-manager.io
}
...
deploy_prism()
{
    ./kustomize build ${MANIFESTS_DIR}/apps/prism/overlays/image-pull-secret | kubectl apply -f - && \
    ./kustomize build ${MANIFESTS_DIR}/bootstrap/components/image-pull-secret/prism | kubectl apply -f -
}
