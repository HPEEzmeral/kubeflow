...
delete_prism()
{
    ./kustomize build ${MANIFESTS_DIR}/bootstrap/components/image-pull-secret/prism | kubectl delete -f -
...
if test_env_vars; then
    ...
    KF_JOBS_NS=${KF_JOBS_NS} ./kustomize build ${MANIFESTS_DIR}/bootstrap/components/installer | kubectl delete -f - -n ${KF_JOBS_NS} --ignore-not-found
