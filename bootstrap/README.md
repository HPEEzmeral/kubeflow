...
| `OIDC_PROVIDER` | The URI which points to dex issuer endpoint for oidc-authservice. | Default value is set to default dex issuer URI. |
| `CA_SECRET_NAME`, `CA_SECRET_KEY` | If oidc-authservice configured for https issuer URI, it is needed to specify CA certificate for oidc-authservice. It will mount cert file to volume in oidc-authservice. `CA_SECRET_NAME` env variable specifies kubernetes secret name where certificate is encoded. `CA_SECRET_KEY` env variable specifies a key name for certificate in a secret. | Default value is empty strings |
